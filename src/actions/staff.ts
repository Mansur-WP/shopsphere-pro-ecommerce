...
import { requireAuth, toNumber } from "@/lib/helpers";
import { productSchema, storeUpdateSchema } from "@/lib/validations";
import type { ActionResult } from "@/types";
import type { OrderStatus } from "@prisma/client";

async function requireApprovedStaff() {
  const session = await requireAuth();

  if (session.user.role === "SUPER_ADMIN" || session.user.role === "ADMIN") {
    return { session, seller: null as null, isSuperAdmin: true };
  }

  // Staff members can have an associated seller/staff profile or manage operational catalog
  const seller = await prisma.sellerProfile.findUnique({
    where: { userId: session.user.id },
  });

  // If role is STAFF and seller profile doesn't exist, create an internal default profile if needed or fallback
  if (!seller && session.user.role === "STAFF") {
    const storeSlug = `staff-${session.user.id.slice(0, 8)}`;
    const createdProfile = await prisma.sellerProfile.create({
      data: {
        userId: session.user.id,
        storeName: session.user.name || "Africhina Staff",
        storeSlug,
        status: "APPROVED",
        businessEmail: session.user.email,
      },
    });
    return { session, seller: createdProfile, isSuperAdmin: false };
  }
  ...
  if (seller.status !== "APPROVED") {
    throw new Error("Staff account is awaiting activation");
  }

  return { session, seller, isSuperAdmin: false };
}

async function verifyProductOwnership(productId: string) {
  const { session, seller, isSuperAdmin } = await requireApprovedStaff();
  ...
  if (!isSuperAdmin && product.sellerId !== seller?.id) {
...
export async function getStaffDashboardStats() {
  try {
    const { seller, isSuperAdmin } = await requireApprovedStaff();

    const whereProduct = isSuperAdmin ? {} : { sellerId: seller?.id };
    const whereOrderItem = isSuperAdmin ? {} : { sellerId: seller?.id };
    const wherePending = isSuperAdmin
      ? { order: { status: "PENDING" as const, paymentStatus: "PAID" as const } }
      : {
          sellerId: seller?.id,
          order: { status: "PENDING" as const, paymentStatus: "PAID" as const },
        };
    ...
    const [
      ...
      prisma.product.count({
        where: { ...whereProduct, published: true },
      }),
    ...
    return {
      ...
      totalRevenue,
      totalSales: toNumber(sellerProfile?.totalSales ?? totalRevenue),
    };
  } catch {
    return null;
  }
...
export async function getStaffProducts(page = 1, limit = 20) {
  try {
    const { seller, isSuperAdmin } = await requireApprovedStaff();
    ...
    const where = isSuperAdmin ? {} : { sellerId: seller?.id };
...
export async function createStaffProduct(
  ...
  try {
    ({ seller, isSuperAdmin } = await requireApprovedStaff());
    if (!seller && !isSuperAdmin) {
      return { success: false, error: "Staff profile required" };
    }
  } catch (e) {
    return {
      success: false,
  ...
  // For super admin without a seller profile, fetch or create a default platform profile
  let targetSellerId = seller?.id;
  if (!targetSellerId) {
    const firstProfile = await prisma.sellerProfile.findFirst();
    targetSellerId = firstProfile?.id;
  }

  if (!targetSellerId) {
    return { success: false, error: "No operational store profile available" };
  }
...
export async function deleteStaffProduct(
  productId: string
): Promise<ActionResult> {
  try {
    await verifyProductOwnership(productId);
...
export async function getStaffOrders(page = 1, limit = 20) {
  try {
    const { seller, isSuperAdmin } = await requireApprovedStaff();
...
export async function updateStaffStore(raw: unknown): Promise<ActionResult> {
  try {
    const { seller } = await requireApprovedStaff();
    if (!seller) {
      return { success: false, error: "Staff profile required" };
    }
...
export async function getStaffAnalytics(days = 30) {
  try {
    const { seller, isSuperAdmin } = await requireApprovedStaff();
    ...
    const whereOrderItem = isSuperAdmin
      ? { createdAt: { gte: since }, order: { paymentStatus: "PAID" as const } }
      : {
          sellerId: seller?.id,
          createdAt: { gte: since },
...
export async function getStaffProduct(productId: string) {
  try {
    const { product } = await verifyProductOwnership(productId);
...
export async function getStaffRecentOrders(limit = 5) {
  const data = await getStaffOrders(1, limit);
  return data?.orders ?? [];
}
...
export async function updateStaffOrderStatus(
  ...
  try {
    ...
    if (!STAFF_ALLOWED_STATUSES.includes(status)) {
      return { success: false, error: "Invalid status for order update" };
    }
